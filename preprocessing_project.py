...
def training_vim_test(train_x, train_y, test_x, test_y, epoch, name):
    ...
    # Assuming train_x, train_y, test_x, and test_y are your input arrays
    train_x_t = torch.tensor(train_x, dtype=torch.float32).to(device)
    train_y_t = torch.tensor(train_y, dtype=torch.float32).to(device)
    test_x_t = torch.tensor(test_x, dtype=torch.float32).to(device)
    test_y_t = torch.tensor(test_y, dtype=torch.float32).to(device)
    ...
    model_save_path = os.path.join(current_dir, 'pengujian_epoch', 'vim_model_4.pth')
...
def plot_vim_combined(loss_values_train, loss_values_val, correlation_values_train, correlation_values_val, num_epochs, time, name):
    # Plotting loss and correlation
    plt.figure(figsize=(12, 5))
...
def plot_loss_cnn(test_losses, train_losses, name):
    font_size = 14
    title_size = 20
...
def main():
    ...
    train_y, test_x, test_y, epoch=500, name='')
    plot_vim_combined(
        train_loss_values, test_loss_values, train_correlation_values, test_correlation_values, num_epochs, time, 
        name='training_performance_vim_500_epoch_0'
    )
    test_mse, test_corr, outputs_flat, targets_flat = test_vim(model, test_x, test_y)
    plot_test_results(outputs_flat, targets_flat, test_corr, test_mse, name='R_performace_vim_500_epoch_0')
    ...
    model.train()
    n_max_epochs = 500 # This is just a super short example. You can increase this.
    max_corr = 0.95
    batch_size = 32
    ...
    # Save the trained model
    model_save_path = os.path.join(current_dir, 'pengujian_epoch', 'cnn_model_4.pth')
    # model_save_path = 'cnn_model.pth'
    torch.save(model.state_dict(), model_save_path)
    print(f'Model saved to {model_save_path}')

    plot_r_cnn(output_test, y_test, name='R_training_performance_cnn_500_epoch_0')
    plot_loss_cnn(test_losses, train_losses, name='loss_training_performance_cnn_500_epoch_0')
