...
def training_vim_test(train_x, train_y, test_x, test_y, epoch, name):
    ...
    model_save_path = os.path.join(current_dir, 'pengujian_split_data', '97,7-2,3', 'vim', 'vim_model_1.pth')
...
def main():
    ...
    # Save the trained model
    model_save_path = os.path.join(current_dir, 'pengujian_split_data', '97,7-2,3', 'cnn', 'cnn_model_1.pth')
    # model_save_path = 'cnn_model.pth'
    torch.save(model.state_dict(), model_save_path)
    print(f'Model saved to {model_save_path}')

    plot_r_cnn(output_test, y_test, name='R_training_performance_cnn_100_epoch_1')
    plot_loss_cnn(test_losses, train_losses, name='loss_training_performance_cnn_100_epoch_1')
